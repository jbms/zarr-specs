...
*Codec*
    ...
    representation of each chunk in the store_. See `chunk encoding`_.
...
- v3 has explicit support for extensions via defined
  `extension points`_ and mechanisms.
...
- v3 introduces `storage transformers`_, which transform reads and writes between
  array and store.
...
- The set of `core data types`_ specified in v3 is less than in v2. Additional
